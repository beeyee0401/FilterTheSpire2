...
public static class BossConfigController
{
    ...
    [
        ...
        (
            nameof(FilterTheSpire2Config.Act3FirstBoss),
            3,
            nameof(FilterTheSpire2Config.Act3SecondBoss)
        ),
        (
            nameof(FilterTheSpire2Config.Act3SecondBoss),
            3,
    ...
    private static void RegisterBossDropdown(
    ...
    private static void RefreshBossDropdown(
        string propName,
        int act,
        string? siblingPropName)
    {
        if (!BossDropdowns.TryGetValue(propName, out var dropdown) ||
        ...
        var validBosses = currentLocation == ActLocations.Any
            ? []
            : BossRules.GetValidBosses(act, currentLocation).ToHashSet();
        var siblingValue = siblingPropName != null
            ? GetBossValue(siblingPropName)
            : BossOptions.Any;
        ...
        var filtered = source
            .Where(item =>
            {
                ...
                if (value == BossOptions.Any)
                {
                    return true;
                }

                if (!validBosses.Contains(value))
                {
                    return false;
                }

                return siblingValue == BossOptions.Any ||
                       value != siblingValue;
            })
        ...
        string? siblingPropName)
    {
        ...
        return new NConfigDropdownItem.ItemData(
            item.Text,
            item.Value,
            () =>
            {
                ...
                // Re-filter the sibling slot so it can no longer target the same
                // boss. For example, Act 3's first and second bosses cannot both
    ...
    private static BossOptions GetBossValue(string propName)
    {
        var property = typeof(FilterTheSpire2Config)
            .GetCachedProperty(propName,
                BindingFlags.Public | BindingFlags.Static);

        return (BossOptions)property!.GetValue(null)!;
    }

    private static void SetBossValue(
        string propName,
        BossOptions value)
    {
        var property = typeof(FilterTheSpire2Config).GetCachedProperty(
                propName,
                BindingFlags.Public | BindingFlags.Static);
        ...
        List<NConfigDropdownItem.ItemData> validItems)
    {
        ...
        return currentValue != BossOptions.Any &&
               validItems.All(item => !Equals(item.Value, currentValue));
    }

    private static void ResetBossOption(
        string propName,
        ...
        List<NConfigDropdownItem.ItemData> filteredItems)
    {
        ...
        var anyItem = filteredItems.FirstOrDefault(item => Equals(item.Value, BossOptions.Any));
